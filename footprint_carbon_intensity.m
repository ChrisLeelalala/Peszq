function [ intensityArr ] = footprint_carbon_intensity( results,genCarbonArr )
%DIRECT_CARBON_INTENSITY Calculate footprint carbon intensity
%   Generate the array of nodal FCI from the mpc file of 
...
for i=1:branchNum
    if branch(i,14)>0
        au(branch(i,2),branch(i,1))=-branch(i,14)/pi(branch(i,1));
    else
        au(branch(i,1),branch(i,2))=-branch(i,16)/pi(branch(i,2));
    end
end
